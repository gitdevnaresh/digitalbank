...
class FCMNotification {
  ...
  registerServices = (onNotificationAction) => {
    ...
    messaging().onTokenRefresh((fcmToken) => {
      console.log("🔄 FCMNotification: Token refreshed!");
      console.log("📱 FCMNotification: New Token:", fcmToken);
      console.log(
        "📱 FCMNotification: New Token (first 50 chars):",
  ...
  createtoken = (onSuccess) => {
    console.log("🔍 FCMNotification: Creating FCM token...");
    messaging()
      .getToken()
      .then((fcmToken) => {
        if (fcmToken) {
          console.log("🎉 FCMNotification: FCM Token created successfully!");
          console.log("📱 FCMNotification: Full Token:", fcmToken);
          console.log(
            "📱 FCMNotification: Token (first 50 chars):",
            fcmToken.substring(0, 50) + "..."
          );
          onSuccess(fcmToken);
        } else {
          console.log("❌ FCMNotification: No FCM token received");
        }
      })
      .catch((err) => {
        console.error("❌ FCMNotification: Error getting FCM token:", err);
      });
  };
